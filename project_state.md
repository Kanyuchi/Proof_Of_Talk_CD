...
**Last updated:** 2026-05-13 (evening) — **Self-enrichment incentive #5 shipped: AI Concierge proactively drafts missing profile fields.** When an attendee opens the chat with < 80% completeness, a tailored welcome offers to GPT-draft the highest-priority missing field (goals → target_companies → interests). Click a candidate, edit inline, Save → embedding regenerates in the background + matches refresh. Maybe-later persists a 30-day decline cooldown so the offer rotates to the next field on the next visit. Smoke-tested end-to-end live in browser against Mona Bauer (50% complete) — DB write, background re-embed, and rotation all verified. With #1 (locked-match preview) and #2 (match-quality benchmark) already live this morning, the self-enrichment loop on the matchmaker is now complete. Earlier today: 11 attendee-side and ops-side fixes shipped (Requests tab, pending-match visibility, register-endpoint fix, sliding-token refresh, etc — see session_log).
