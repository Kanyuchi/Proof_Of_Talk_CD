...
**Last updated:** 2026-05-04 — **Master speaker sheet ingested**. 143 new SPEAKER/VIP attendees from the POT26 master Speaker Tracking Google Sheet (sheet ID `1DJJ5vQ-…`); DB went 172 → 315 attendees (132 SPEAKER, 129 DELEGATE, 54 VIP). New `app/services/speakers_sheet_sync.py` replaces the old `speakers_sync.py` cron path (which read a near-empty 8-row Supabase table). Ingest script at `backend/scripts/ingest_speakers_sheet.py`. Enrichment + match generation for the new rows running now. Phase 2 #1 (free-slot visibility) still awaiting Railway deploy.
