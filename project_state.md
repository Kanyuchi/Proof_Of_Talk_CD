...
**Last updated:** 2026-05-01 (Phase 2 #1 shipped — Free-slot visibility on match cards — awaiting Railway deploy. Backend `app/services/slots.py` + `MatchResponse.mutual_free_slots` + 409 conflict check; frontend chip strip on `MyMatches`. Next up Phase 2 #2: mutual-match in-app inbox.)
