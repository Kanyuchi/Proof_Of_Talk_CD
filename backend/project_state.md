...
## What's Working
- [sync] Nightly crons resilient to Supabase connection drops via `core/database.py::run_with_db_retry` (fresh session + retry-once). grid_audit terminal write, match_refresh target fetch, and extasy chunk loop wrapped. extasy now persists `stats.error_reasons`; speaker placeholder-email collisions handled as noop. Fixed 2026-05-21 (commit 8293c9b) and smoke-tested green on prod (20e8026) — all 4 jobs flipped sync_status to ok.
