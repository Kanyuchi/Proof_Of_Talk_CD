...
1. **Verify tomorrow's 02:00–02:45 UTC scheduler run on Railway** — first end-to-end proof of the three fixes shipped today (match refresh, `ticket_bought_at`, grid audit 401). Check `grid_audit_runs.run_at` ≥ 02:30 with non-zero totals; check `attendees.ticket_bought_at` populated for any new arrivals; check `matches` table grew if there were new arrivals.
2. **Change Extasy sync cadence to every 5h** — now that today's fixes are live, edit `backend/app/main.py:62` from `CronTrigger(hour=2, minute=0)` to `IntervalTrigger(hours=5)` per Karl's request. (Keep speakers/grid/match jobs on daily 02:15/02:30/02:45 — only Extasy needs the higher cadence.) Validate after one cycle that all three downstream jobs still fire correctly.
