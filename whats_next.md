...
- **Photo-upload nudge inside the AI Concierge offer** *(follow-on to #5 — shipped 2026-05-13 evening)* — `photo_url` is in the completeness denominator (6 fields) but NOT in the GPT-drafted offer set, by design. If a user is at 5/6 with only photo missing, the offer correctly returns `null` today. Build a separate non-GPT photo nudge that surfaces inside the same `ProfilePromptOffer` component: file-upload affordance + "Skip" button, no LLM call. Hooks into the existing `attendees.photo_url` write. ~30 min.
